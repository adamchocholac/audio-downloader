...
def upload_to_archive(
    ...
    Streams the upload in chunks so on_progress receives real percentage updates:
    ...
    headers = {
        ...
        "x-archive-meta-description": f"Audio downloaded from YouTube: {title}",
        "x-archive-size-hint":        str(file_size),
        "x-archive-queue-derive":     "0",  # skip derivative processing → faster
    }
    ...
    def _stream():
        ...
        with open(file_path, "rb") as f:
            while True:
                ...
                uploaded += len(chunk)
                if on_progress:
                    pct = round(uploaded / file_size * 100, 1)
                    on_progress({"phase": "uploading", "percent": pct})
                yield chunk
...
def public_url(identifier: str, filename: str) -> str:
    return f"https://archive.org/download/{identifier}/{filename}"
